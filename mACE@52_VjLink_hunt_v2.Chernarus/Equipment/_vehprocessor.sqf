...
call compile format ["if (isNil {SerP_veh_%1_processor}) then {SerP_veh_%1_processor = compile preprocessFileLineNumbers 'equipment\%1.sqf'}",_faction];
...
_veh call _addTyre;
[_veh, _loadout] call compile format ["if isNil {SerP_veh_%1_processor} then {SerP_veh_%1_processor = compile preprocessFileLineNumbers 'equipment\%1.sqf'}; _this call SerP_veh_%1_processor",_faction];
